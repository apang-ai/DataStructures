...
一、树相关的概念：
    节点： 节点是树的基本部分，也可以叫‘键’。节点可以有附加信息。这个附加信息叫做“有效载荷”
    边：边是树的另一个基本部分，边连接两个节点已显示他们之间存在的关系，每个节点（除了跟之外），都恰好从另一个节点传入连接。每个节点可以具有多个输出边
    根：树的跟是树中唯一没有传入边的节点
    路径：路径是由边连接节点的有序列表
    子节点：具有来自相投传入边的节点C的集合成为该节点的子节点
    父节点：具有和它相同传入边的所连接的节点为父节点
    兄弟节点：树中作为同一父节点的子节点的节点被成为兄弟节点
...
二、树的定义：
    定义一、书友一组节点和一组链接节点的边组成
            树的一个节点被指定为根节点
            除了根节点之外，每个节点n通过一个其他节点p的边连接，其中p是n的父节点

            从根路径遍历到每个节点路径唯一
            如果树中的每个节点最多有两个子节点，我们就概述是一个“二叉树”
    定义二（递归定义）：
        书是空的，或者由一个根节点和0个或者多个子树组成，每个子树也是一棵树，每个子树的节点通过边连接到父树的根节点。
...
三、树的表示
    ...
    2、节点表示：（抽象ADT）  使用节点和引用，定义一个具有根属性的类和左子树，右子树
...
如何从待带有完全括号的数学表达式 ( (7+3)*(5-2) ) 去构建分析树
怎么放入？
如何取出？
# 分析：根节点，左子树，右子树
第一步：把表达式字符串拆分成富豪列表。四种：左括号，右括号，运算符，操作数
        读到一个左括号：开始新的表达式，树对应表达式的话，此时应当创建一个新的树
        读到一个右括号：结束了一个表达式
...
第二步： 根据第一步分析，定义规则
        如果当前符号是左括号，需要添加一个新的节点，作为当前节点的左子节点，并且下降到左子节点
        如果当前的符号是列表['+','-','*','/']中，将当前节点的根值设置为由当前符号表示的运算法，添加一个新节点作为当前节点的右子节点，并下降到右子节点
        如果当前的符号是数字，将当前节点的根值设置为该数字并返回父节点
        如果当前符号是")"，则转到当前节点的父节点
